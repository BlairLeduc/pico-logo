# Graph Report - pico-logo  (2026-08-06)

## Corpus Check
- 298 files · ~534,688 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7303 nodes · 22529 edges · 208 communities (198 shown, 10 thin omitted)
- Extraction: 57% EXTRACTED · 43% INFERRED · 0% AMBIGUOUS · INFERRED: 9694 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3a7cbd04`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- run_string
- eval_string
- lfs.c
- test_value.c
- reset_output
- mem_word_ptr
- define_proc
- mem_is_nil
- mem_atom
- value_to_string
- lexer_init
- format_buffer_init
- test_mock_device.c
- mem_cons
- syntax_highlight_line
- primitives.h
- test_primitives_editor.c
- test_frame_arena.c
- test_httpd.c
- proc_define_from_text
- primitives_turtle.c
- io.c
- result_error_arg
- test_io.c
- unity.c
- test_primitives_http.c
- picocalc_console.c
- mock_device.c
- httpd.c
- Turtle Graphics
- test_tilemap.c
- fat32.c
- test_scaffold.h
- test_primitives_json.c
- test_primitives_wifi.c
- test_mock_fs.h
- result_none
- test_scaffold_setUp
- test_checkrun.c
- test_variables.c
- test_primitives_files_load_save.c
- error_format
- test_primitives_conditionals.c
- test_trails.c
- value_number
- lexer_next_token
- test_cross_fs_move.c
- memory.c
- test_primitives_files.c
- test_primitives_hardware.c
- set_mock_input
- test_time.c
- repository
- Checkpoint Run — a maze-driving game (design)
- picocalc_editor_edit
- test_primitives_files_directory.c
- test_primitives_outside_world.c
- stream.c
- test_primitives_tilemap.c
- test_mklfsimg.c
- demons_poll
- stdlib.h
- primitives_text.c
- frame.c
- test_notation.c
- test_lfs_backup.c
- lcd.c
- ;
- test_sound.c
- fat32_close
- PicoCalc hardware integration — what we know
- test_primitives_network.c
- op_stack_depth
- primitives_json.c
- LOGO_HOT
- P9 — Tile maps and smooth scrolling (design)
- primitives_init
- picocalc_hardware.c
- picocalc_read_line
- Words and Lists
- Conditionals and Control of Flow
- primitives_arithmetic.c
- screen_set_mode
- test_dirty_tiles.c
- P10 — Interpreter throughput (design)
- lexer.c
- Turtle Trails (design)
- P5 — Multi-sprite turtles and the display pipeline (implemented)
- procedures.c
- lfs_storage.c
- keyboard.c
- mock_sdcard.c
- test_galaxian.c
- picocalc_storage.c
- P8 — Sound: a stereo PSG synthesizer (design)
- main
- mem_atom_cstr
- test_storage_router.c
- primitives_sound.c
- test_primitives_debug_control.c
- host_storage.c
- test_costumes.c
- properties.c
- test_primitives_debug.c
- test_lfs_storage.c
- host_hardware.c
- Code Review — 2026-07-02
- Design: LittleFS internal filesystem + `/sd` FAT32 mount
- Contributing
- primitives_http.c
- repl_evaluate_line
- The pick of five: plans
- Arithmetic Operations
- Space Invaders in Pico Logo (design & implementation)
- package.json
- roadmap.md
- primitives.c
- sdcard.c
- Managing your Workspace
- test_invaders.c
- test_scaffold.c
- eval.c
- test_help.c
- test_tls_heap.c
- storage_router.c
- read_map
- File Management
- The Outside World
- picocalc_flash.c
- sound.c
- Galaxian in Pico Logo (design)
- test_bench_throughput.c
- primitives_control_flow.c
- primitives_outside_world.c
- frame_get_test
- frame_stack_depth
- clib.c
- Atom Garbage Collection: Implementation Plan
- Input and Output to Files, Network Connections and Devices
- record_command
- LOGO_HOT
- mem_set_cdr
- primitives_debug_control.c
- ensure_wifi_initialized
- picocalc_network_tls_connect
- southbridge.c
- Design: `launch` background processes (P6)
- host_console.c
- logo_io_start_dribble
- Using the Logo Editor
- WiFi Management
- HTTP Server
- primitives_conditionals.c
- Concurrent present on core 1 (design)
- HTTP server (design)
- MockCommandType
- Introduction
- record_command_float
- repl_line_starts_with_to
- ms_to_datetime
- Text and Screen Commands
- primitives_bitwise.c
- LogoStream
- logo_hardware_init
- PR Review Checklist (CRITICAL)
- List Processing
- CLAUDE.md
- as_httpd_conn
- Appendix A: Useful Tools
- Appendix B: Parsing
- Modifying Procedures Under Program Control
- Managing Various Files
- HTTP Operations
- Tile Maps
- Bitwise Operations
- Variables
- Pico Logo
- Time Management
- JSON
- drain_tokens
- Bugs
- gen_ca_certs.py
- pandoc_slug
- iteration_callback
- mock_device_set_raster
- dist.sh
- mock_device_get_dot
- generate_help.sh
- run_e2e.sh
- LogoEditorResult
- mock_sound_queue
- mock_sound_status
- mock_wifi_status
- mock_text_set_width
- VENDOR.md

## God Nodes (most connected - your core abstractions)
1. `run_string()` - 994 edges
2. `eval_string()` - 928 edges
3. `mem_word_ptr()` - 446 edges
4. `mem_atom()` - 242 edges
5. `mem_is_nil()` - 240 edges
6. `value_to_string()` - 208 edges
7. `result_error_arg()` - 199 edges
8. `result_none()` - 193 edges
9. `result_ok()` - 177 edges
10. `lexer_init()` - 173 edges

## Surprising Connections (you probably didn't know these)
- `test_car_of_nil()` --calls--> `mem_is_nil()`  [INFERRED]
  tests/test_memory.c → core/memory.c
- `test_cdr_of_nil()` --calls--> `mem_is_nil()`  [INFERRED]
  tests/test_memory.c → core/memory.c
- `test_nil_is_nil()` --calls--> `mem_is_nil()`  [INFERRED]
  tests/test_memory.c → core/memory.c
- `test_nil_is_not_word()` --calls--> `mem_is_word()`  [INFERRED]
  tests/test_memory.c → core/memory.c
- `test_value_number_content()` --calls--> `value_number()`  [INFERRED]
  tests/test_value.c → core/value.c

## Import Cycles
- None detected.

## Communities (208 total, 10 thin omitted)

### Community 0 - "run_string"
Cohesion: 0.02
Nodes (235): MockDeviceState, mock_device_get_output(), mock_device_get_state(), mock_device_has_line_from_to(), mock_device_verify_palette(), test_cleartext_clears_screen(), test_cs_restores_auto_refresh(), test_cs_restores_from_sync() (+227 more)

### Community 1 - "eval_string"
Cohesion: 0.02
Nodes (183): test_abs_decimal(), test_abs_negative(), test_abs_positive(), test_abs_zero(), test_arctan(), test_arctan_too_many_inputs(), test_arctan_two_input(), test_arctan_two_input_vertical() (+175 more)

### Community 2 - "lfs.c"
Cohesion: 0.06
Nodes (184): lfs1_dir_t, lfs1_entry_t, lfs_cache_t, lfs_dir_t, lfs_file_t, lfs_gstate_t, lfs_mdir_t, lfs_soff_t (+176 more)

### Community 3 - "test_value.c"
Cohesion: 0.02
Nodes (165): format_number(), Result, Value, result_get_error_code(), result_get_goto_label(), result_get_pause_proc(), result_get_throw_tag(), result_get_value() (+157 more)

### Community 4 - "reset_output"
Cohesion: 0.02
Nodes (141): test_rerandom_affects_pick_and_shuffle(), test_comment_in_procedure(), test_comment_inline(), test_comment_with_list(), test_comment_with_word(), test_deep_nested_proc_in_repeat(), test_do_until_basic(), test_do_until_invalid_predicate() (+133 more)

### Community 5 - "mem_word_ptr"
Cohesion: 0.02
Nodes (134): Value, value_is_true(), mem_word_ptr(), parse_list_from_string(), ParseListResult, bind_long_blob_word(), exhaust_atom_table(), exhaust_node_pool() (+126 more)

### Community 6 - "define_proc"
Cohesion: 0.02
Nodes (126): test_b10_bare_exponent_in_list_path_is_not_a_number(), test_b10_bare_n_exponent_word_is_not_a_number(), test_b10_valid_exponent_forms_still_numbers(), test_b7_grouped_call_inside_operand(), test_b7_grouped_multi_branch_call_inside_operand(), test_b7_grouping_paren_starting_with_call(), test_b7_grouping_paren_starting_with_call_output(), test_b7_nested_grouping_parens_around_call() (+118 more)

### Community 7 - "mem_is_nil"
Cohesion: 0.06
Nodes (105): Node, mem_atom_unescape(), mem_first_cell(), mem_gc_roots_pop(), mem_gc_roots_push(), mem_is_nil(), mem_is_word(), mem_next_cell() (+97 more)

### Community 8 - "mem_atom"
Cohesion: 0.05
Nodes (95): mem_atom(), mem_blob(), mem_blob_free_bytes(), mem_blob_used(), mem_free_atoms(), mem_free_nodes(), mem_gc(), mem_is_blob() (+87 more)

### Community 9 - "value_to_string"
Cohesion: 0.10
Nodes (95): read_predicate_bool(), number_to_word(), mem_list_append(), mem_set_car(), mem_word(), mem_word_len(), Evaluator, Result (+87 more)

### Community 10 - "lexer_init"
Cohesion: 0.06
Nodes (92): lexer_init(), assert_token(), test_alphanumeric_word(), test_bar_colon_variable(), test_bar_escaped_bar_inside(), test_bar_in_list_context(), test_bar_quoted_word(), test_bar_run_mid_quoted_word() (+84 more)

### Community 11 - "format_buffer_init"
Cohesion: 0.07
Nodes (85): Node, UserProcedure, Value, format_body_element(), format_body_element_multiline(), format_body_indent(), format_buffer_init(), format_buffer_output() (+77 more)

### Community 12 - "test_mock_device.c"
Cohesion: 0.04
Nodes (84): LogoConsole, LogoStreamOps, logo_console_has_editor(), logo_console_has_screen_modes(), logo_console_has_text(), logo_console_has_turtle(), logo_console_init(), MockCommand (+76 more)

### Community 13 - "mem_cons"
Cohesion: 0.07
Nodes (83): atom_memo_class(), atom_memo_set_class(), mem_cons(), Lexer, Node, Token, TokenType, compute_word_class() (+75 more)

### Community 14 - "syntax_highlight_line"
Cohesion: 0.06
Nodes (80): bracket_category(), SyntaxCategory, ci_eq(), is_delimiter(), match_keyword(), read_word_span(), scan_comment(), scan_number() (+72 more)

### Community 15 - "primitives.h"
Cohesion: 0.06
Nodes (56): Node, demons_clear(), demons_freeze(), demons_print(), demons_reset(), demons_resume(), demons_set(), demons_suspend() (+48 more)

### Community 16 - "test_primitives_editor.c"
Cohesion: 0.07
Nodes (77): mock_device_clear_editor(), mock_device_get_editor_input(), mock_device_set_editor_content(), mock_device_set_editor_result(), mock_device_was_editor_called(), LogoDirCallback, LogoStream, MockFile (+69 more)

### Community 17 - "test_frame_arena.c"
Cohesion: 0.07
Nodes (76): arena_alloc_words(), arena_available(), arena_available_bytes(), arena_capacity(), arena_capacity_bytes(), arena_extend(), arena_free_to(), arena_init() (+68 more)

### Community 18 - "test_httpd.c"
Cohesion: 0.06
Nodes (72): httpd_listening(), httpd_request_pending(), mock_httpd_conn_response(), mock_httpd_is_listening(), mock_httpd_listen_port(), mock_httpd_queue_connection(), mock_httpd_queue_connection_ex(), mock_httpd_queue_connection_stalled() (+64 more)

### Community 19 - "proc_define_from_text"
Cohesion: 0.05
Nodes (76): proc_define_from_text(), FrameStack, proc_get_frame_stack(), test_b9_multiline_literal_counts_as_data(), test_b9_multiline_literal_iterates_as_data(), test_b9_text_count_ignores_layout(), test_b9_text_item_ignores_layout(), test_butlast_removes_last_line() (+68 more)

### Community 20 - "primitives_turtle.c"
Cohesion: 0.14
Nodes (76): arg_turtle(), Evaluator, LogoConsoleTurtle, LogoTurtleRaster, Result, Value, get_turtle_ops(), make_position_list() (+68 more)

### Community 21 - "io.c"
Cohesion: 0.08
Nodes (75): LogoDirCallback, LogoIO, LogoStream, SyntaxCategory, highlight_write_span(), logo_io_check_freeze_request(), logo_io_check_pause_request(), logo_io_check_user_interrupt() (+67 more)

### Community 22 - "result_error_arg"
Cohesion: 0.11
Nodes (67): CatalogContext, CatalogEntry, Evaluator, Result, Value, Evaluator, LogoEntryType, LogoIO (+59 more)

### Community 23 - "test_io.c"
Cohesion: 0.04
Nodes (50): logo_io_parse_network_address(), logo_io_set_prefix(), logo_io_set_writer(), LogoDirCallback, LogoEntryType, LogoStream, dribble_flush_fn(), mock_dir_callback() (+42 more)

### Community 24 - "unity.c"
Cohesion: 0.12
Nodes (65): IsStringInBiggerString(), UnityAddMsgIfSpecified(), UnityAssertBits(), UnityAssertDoublesNotWithin(), UnityAssertDoubleSpecial(), UnityAssertDoublesWithin(), UnityAssertEqualIntArray(), UnityAssertEqualMemory() (+57 more)

### Community 25 - "test_primitives_http.c"
Cohesion: 0.06
Nodes (66): logo_mem_set_aux_region(), mock_device_get_last_tcp_ip(), mock_device_get_last_tcp_port(), mock_device_get_last_tls_host(), mock_device_get_tcp_request(), mock_device_set_tcp_connect_result(), mock_device_set_tcp_response(), Result (+58 more)

### Community 26 - "picocalc_console.c"
Cohesion: 0.06
Nodes (52): LogoPen, LogoRotationStyle, LogoStream, LogoTurtleRaster, ScreenSprite, error_output_flush(), error_output_write(), heading_faces_left() (+44 more)

### Community 27 - "mock_device.c"
Cohesion: 0.03
Nodes (13): mock_device_get_tcp_request_len(), mock_device_set_input(), mock_device_set_snap_result(), mock_device_set_tcp_close_after(), mock_device_set_tcp_read_chunk(), mock_device_set_tcp_write_chunk(), mock_device_was_restore_palette_called(), mock_httpd_set_read_hook() (+5 more)

### Community 28 - "httpd.c"
Cohesion: 0.08
Nodes (64): Result, demons_maybe_poll(), demons_running(), LogoHardwareOps, Result, Value, check_response_headers(), ci_eq() (+56 more)

### Community 29 - "Turtle Graphics"
Cohesion: 0.03
Nodes (67): arc, ask, back (bk), background (bg), clean, cleardemons, clearscreen (cs), colourunder (colorunder) (+59 more)

### Community 30 - "test_tilemap.c"
Cohesion: 0.11
Nodes (60): bake_ready(), bake_rect(), Evaluator, LogoConsoleTurtle, Result, Value, get_turtle_ops(), is_int_in() (+52 more)

### Community 31 - "fat32.c"
Cohesion: 0.11
Nodes (61): allocate_and_link_cluster(), fat32_error_t, clear_cluster(), cluster_to_sector(), delete_entry(), dir_offset_to_location(), fat32_delete(), fat32_dir_create() (+53 more)

### Community 32 - "test_scaffold.h"
Cohesion: 0.03
Nodes (56): tearDown(), tearDown(), tearDown(), tearDown(), tearDown(), tearDown(), tearDown(), tearDown() (+48 more)

### Community 33 - "test_primitives_json.c"
Cohesion: 0.07
Nodes (62): assert_empty(), assert_number(), assert_word(), Result, make_doc(), test_array_index_is_one_based(), test_array_of_objects(), test_boolean_true() (+54 more)

### Community 34 - "test_primitives_wifi.c"
Cohesion: 0.05
Nodes (60): mock_device_add_wifi_scan_result(), mock_device_clear_wifi_scan_results(), mock_device_get_hostname(), mock_device_set_wifi_connect_result(), mock_device_set_wifi_connected(), mock_device_set_wifi_ip(), mock_device_set_wifi_mac(), mock_device_set_wifi_scan_result() (+52 more)

### Community 35 - "test_mock_fs.h"
Cohesion: 0.07
Nodes (58): LogoDirCallback, LogoStream, MockFile, mock_file_can_read(), mock_file_close(), mock_file_flush(), mock_file_get_length(), mock_file_get_read_pos() (+50 more)

### Community 36 - "result_none"
Cohesion: 0.11
Nodes (58): prim_editfile(), Evaluator, Result, Value, prim_load(), prim_loadpic(), prim_pofile(), prim_save() (+50 more)

### Community 37 - "test_scaffold_setUp"
Cohesion: 0.05
Nodes (58): LogoIO, primitives_control_reset_test_state(), primitives_set_io(), properties_init(), variables_init(), main(), LogoStorage, logo_host_storage_create() (+50 more)

### Community 38 - "test_checkrun.c"
Cohesion: 0.10
Nodes (58): MockStamp, mock_device_clear_graphics(), mock_device_get_stamp(), mock_device_stamp_count(), check_world_invariants(), load_checkrun(), num(), numf() (+50 more)

### Community 39 - "test_variables.c"
Cohesion: 0.07
Nodes (56): find_global(), var_bury(), var_declare_local(), var_erase(), var_erase_all(), var_erase_all_globals(), var_is_shadowed_by_local(), var_set_local() (+48 more)

### Community 40 - "test_primitives_files_load_save.c"
Cohesion: 0.05
Nodes (57): var_exists(), mock_device_get_gfx_load_call_count(), mock_device_get_gfx_save_call_count(), mock_device_get_last_gfx_load_filename(), mock_device_get_last_gfx_save_filename(), mock_device_set_gfx_load_result(), mock_device_set_gfx_save_result(), setUp_with_turtle() (+49 more)

### Community 41 - "error_format"
Cohesion: 0.06
Nodes (50): CaughtError, append_caller_suffix(), Result, error_clear_caught(), error_format(), error_get_caught(), error_message(), error_set_caught() (+42 more)

### Community 42 - "test_primitives_conditionals.c"
Cohesion: 0.04
Nodes (51): test_if_false_case_insensitive(), test_if_false_one_list_command(), test_if_false_two_lists_command(), test_if_list_predicate_error(), test_if_list_with_empty_list_arg(), test_if_list_with_output(), test_if_list_with_print_empty_then_stop(), test_if_list_with_stop() (+43 more)

### Community 43 - "test_trails.c"
Cohesion: 0.14
Nodes (51): actor(), load_logo(), load_trails(), num(), put_actor(), run(), runf(), setUp() (+43 more)

### Community 44 - "value_number"
Cohesion: 0.10
Nodes (49): frame_clear_continuation(), frame_current(), frame_get_bindings(), frame_has_pending_continuation(), frame_push_value(), frame_save_continuation(), frame_stack_used_bytes(), frame_value_count() (+41 more)

### Community 45 - "lexer_next_token"
Cohesion: 0.07
Nodes (50): lexer_next_token(), lexer_token_text(), assert_token_type(), TokenType, test_digit_starting_word(), test_fuzz_all_operators_consecutive(), test_fuzz_backslash_before_delimiter(), test_fuzz_binary_mixed_with_delimiters() (+42 more)

### Community 46 - "test_cross_fs_move.c"
Cohesion: 0.07
Nodes (36): logo_io_copy_file(), MemFile, bd_erase(), bd_prog(), bd_read(), lfs_block_t, lfs_off_t, lfs_size_t (+28 more)

### Community 47 - "memory.c"
Cohesion: 0.09
Nodes (46): BlobDesc, demons_gc_mark_all(), remember_binding(), alloc_cell(), atom_chain_next(), atom_clear_marks(), atom_entry_is_free(), atom_entry_next() (+38 more)

### Community 48 - "test_primitives_files.c"
Cohesion: 0.04
Nodes (47): test_append_to_file(), test_close_file(), test_close_invalid_input(), test_close_unopened_file_error(), test_dribble_starts(), test_filelen_empty_file(), test_filelen_invalid_input(), test_filelen_returns_size() (+39 more)

### Community 49 - "test_primitives_hardware.c"
Cohesion: 0.05
Nodes (47): tearDown(), test_battery_charging(), test_battery_charging_in_procedure(), test_battery_in_procedure(), test_battery_level_empty(), test_battery_level_full(), test_battery_level_partial(), test_battery_level_unavailable() (+39 more)

### Community 50 - "set_mock_input"
Cohesion: 0.15
Nodes (43): LogoIO, repl_cleanup(), repl_extract_proc_name(), repl_init(), repl_run(), ReplFlags, test_repl_defines_proc_with_multiline_paren(), test_repl_error_clears_sync_refresh() (+35 more)

### Community 51 - "test_time.c"
Cohesion: 0.07
Nodes (45): mock_device_set_time(), mock_device_set_time_enabled(), test_date_and_setdate_roundtrip(), test_date_error_when_not_available(), test_date_outputs_correct_day(), test_date_outputs_correct_month(), test_date_outputs_correct_year(), test_date_outputs_different_values() (+37 more)

### Community 52 - "repository"
Cohesion: 0.04
Nodes (45): name, name, match, name, 1, 2, match, name (+37 more)

### Community 53 - "Checkpoint Run — a maze-driving game (design)"
Cohesion: 0.05
Nodes (44): 10.1 Radar, 10.2 HUD, 10.3 Palette, 10.4 Shape slots, 10.5 Sound, 10. Radar, HUD, art, and sound, 11. State machine and frame order, 12. Logo coding constraints (+36 more)

### Community 54 - "picocalc_editor_edit"
Cohesion: 0.17
Nodes (42): LogoEditorResult, editor_backspace(), editor_compute_depth_at_line(), editor_copy_line(), editor_copy_selection(), editor_count_lines(), editor_cut_line(), editor_decrease_indent() (+34 more)

### Community 55 - "test_primitives_files_directory.c"
Cohesion: 0.06
Nodes (41): mock_fs_create_dir(), test_cat_lists_files(), test_cat_runs_without_error(), test_cat_with_invalid_input_error(), test_catalog_long_format_marks_directories(), test_catalog_long_format_shows_size(), test_catalog_runs_without_error(), test_catalog_with_absolute_pathname() (+33 more)

### Community 56 - "test_primitives_outside_world.c"
Cohesion: 0.05
Nodes (41): test_keyp_no_input_returns_false(), test_keyp_with_input_returns_true(), test_pr_abbreviation(), test_print_empty_list(), test_print_list_no_outer_brackets(), test_print_multiple_args(), test_print_nested_list(), test_print_number() (+33 more)

### Community 57 - "stream.c"
Cohesion: 0.10
Nodes (38): create_network_stream(), logo_io_check_write_error(), LogoStream, screen_gfx_load(), screen_gfx_save(), LogoStream, LogoStreamOps, logo_stream_can_read() (+30 more)

### Community 58 - "test_primitives_tilemap.c"
Cohesion: 0.09
Nodes (40): mock_device_get_canvas_point(), mock_device_paint_canvas(), mock_device_set_canvas_point(), assert_baked_tile(), assert_canvas_block(), capture_tile(), tearDown(), test_bake_paints_empty_cells_in_the_background_colour() (+32 more)

### Community 59 - "test_mklfsimg.c"
Cohesion: 0.08
Nodes (33): bd_erase(), bd_prog(), bd_read(), blob_get_read_pos(), blob_read_chars(), blob_set_read_pos(), lfs_block_t, lfs_off_t (+25 more)

### Community 60 - "demons_poll"
Cohesion: 0.10
Nodes (37): demons_frozen(), demons_poll(), MockTurtleState, mock_device_clear_output(), mock_device_get_turtle(), tearDown(), test_action_does_not_reenter_poll(), test_cleardemons_disarms_all() (+29 more)

### Community 61 - "stdlib.h"
Cohesion: 0.07
Nodes (29): text_get_background(), text_get_foreground(), text_set_background(), turtle_canvas_point(), turtle_canvas_write_row(), turtle_dot_at(), turtle_draw_text(), turtle_fill() (+21 more)

### Community 62 - "primitives_text.c"
Cohesion: 0.14
Nodes (35): frame_sync_active(), frame_sync_period(), frame_sync_reset(), frame_sync_set(), frame_sync_wait_ms(), Evaluator, Result, Value (+27 more)

### Community 63 - "frame.c"
Cohesion: 0.11
Nodes (38): Binding, FrameHeader, FrameStack, UserProcedure, Value, word_offset_t, calc_frame_size(), frame_at() (+30 more)

### Community 64 - "test_notation.c"
Cohesion: 0.12
Nodes (33): NotationState, SoundEvent, duration_ms(), notation_parse_token(), notation_state_init(), note_freq(), parse_control(), pitch_class() (+25 more)

### Community 65 - "test_lfs_backup.c"
Cohesion: 0.12
Nodes (33): lfs_block_t, lfs_t, LogoStream, get_u32(), logo_lfs_backup(), logo_lfs_restore(), mark_block(), put_u32() (+25 more)

### Community 66 - "lcd.c"
Cohesion: 0.11
Nodes (35): repeating_timer_t, decode_char(), lcd_blit(), lcd_blit_begin(), lcd_blit_end(), lcd_clear_screen(), lcd_cursor_blink(), lcd_cursor_enabled() (+27 more)

### Community 67 - ";"
Cohesion: 0.05
Nodes (38): ;, and, Appendix C: Useful Procedures, Appendix D: Error Messages, Appendix E: Colour Palette for Pico Logo, battery, .bootsel, Break (+30 more)

### Community 68 - "test_sound.c"
Cohesion: 0.08
Nodes (36): mock_sound_set_status(), assert_number_list(), assert_word(), MockDeviceState, Result, Value, snd(), test_env_default() (+28 more)

### Community 69 - "fat32_close"
Cohesion: 0.14
Nodes (35): repeating_timer_t, fat32_close(), fat32_create(), fat32_is_mounted(), fat32_mount(), fat32_read(), fat32_unmount(), is_valid_fat32_boot_sector() (+27 more)

### Community 70 - "PicoCalc hardware integration — what we know"
Cohesion: 0.05
Nodes (37): 10. Checklist for the new engine, 1.1 SRAM is the scarce resource, 1.2 PSRAM (Plus 2 W only), 1. The machine, 2.1 What the panel is, 2.2 The blit pipeline, 2.3 Measured throughput — the single most important number, 2.4 Hardware vertical scroll (+29 more)

### Community 71 - "test_primitives_network.c"
Cohesion: 0.11
Nodes (35): mock_device_get_last_ntp_server(), mock_device_get_last_ntp_timezone(), mock_device_get_last_ping_ip(), mock_device_get_last_resolve_hostname(), mock_device_set_ntp_result(), mock_device_set_ping_result(), mock_device_set_resolve_result(), test_http_get_dns_failure_errors() (+27 more)

### Community 72 - "op_stack_depth"
Cohesion: 0.12
Nodes (31): EvalOp, Value, LOGO_HOT(), mark_value(), op_stack_alloc_prim_args(), op_stack_depth(), op_stack_gc_mark(), op_stack_get_prim_args() (+23 more)

### Community 73 - "primitives_json.c"
Cohesion: 0.17
Nodes (35): Evaluator, Node, Result, Value, enter_array(), enter_object(), extract_value(), hex_val() (+27 more)

### Community 74 - "LOGO_HOT"
Cohesion: 0.16
Nodes (34): UserProcedure, eval_at_end(), advance(), get_infix_bp(), Evaluator, Token, TokenType, peek() (+26 more)

### Community 75 - "P9 — Tile maps and smooth scrolling (design)"
Cohesion: 0.06
Nodes (35): 10. Checkpoint Run revamp, 11. Turtle Trails revamp (render-only, gameplay identical), 12. Budgets, 13.1 M1+M2 as built (2026-08-02), 13.2 M1+M2 on hardware (2026-08-02, Pico Plus 2 W), 13.3 M3 as built (2026-08-02), 13.4 M3 on hardware (2026-08-02) — accepted, but on the wrong board, 13.5 The board mismatch, and how to settle it (+27 more)

### Community 76 - "primitives_init"
Cohesion: 0.11
Nodes (34): primitives_arithmetic_init(), primitives_bitwise_init(), primitives_conditionals_init(), primitives_control_flow_init(), primitives_debug_control_init(), primitives_debug_init(), primitives_editor_init(), primitives_events_init() (+26 more)

### Community 77 - "picocalc_hardware.c"
Cohesion: 0.07
Nodes (15): cyw43_ev_scan_result_t, mbedtls_ms_time(), mdns_stop(), ntp_dns_callback(), ntp_send_request(), picocalc_sleep(), picocalc_wifi_disconnect(), tcp_client_connected_cb() (+7 more)

### Community 78 - "picocalc_read_line"
Cohesion: 0.11
Nodes (27): history_add(), history_get(), history_get_start_index(), history_is_empty(), history_is_end_index(), history_next_index(), history_next_matching(), history_prev_index() (+19 more)

### Community 79 - "Words and Lists"
Cohesion: 0.06
Nodes (34): ascii, before? (beforep), butfirst (bf), butlast (bl), char, count, empty? (emptyp), equal? (equalp) (+26 more)

### Community 80 - "Conditionals and Control of Flow"
Cohesion: 0.06
Nodes (34): catch, co, Conditionals and Control of Flow, do.until, do.while, error, false, for (+26 more)

### Community 81 - "primitives_arithmetic.c"
Cohesion: 0.19
Nodes (30): Evaluator, Result, Value, prim_abs(), prim_arctan(), prim_cos(), prim_difference(), prim_exp() (+22 more)

### Community 82 - "screen_set_mode"
Cohesion: 0.10
Nodes (30): picocalc_editor_get_ops(), lcd_enable_cursor(), lcd_get_palette_value(), lcd_restore_palette(), lcd_set_background(), lcd_set_palette_rgb(), lcd_set_palette_value(), LogoConsole (+22 more)

### Community 83 - "test_dirty_tiles.c"
Cohesion: 0.14
Nodes (29): dirty_tiles_any(), dirty_tiles_clear(), dirty_tiles_mark_all(), dirty_tiles_mark_rect(), dirty_tiles_mark_rect_wrap(), dirty_tiles_next_span(), wrap_coord(), ScreenSprite (+21 more)

### Community 84 - "P10 — Interpreter throughput (design)"
Cohesion: 0.06
Nodes (33): 10. Relationship to P9, 11.1 First profile (2026-08-02, Pico Plus 2 W, 200 frames), 11.2 The instruction-fetch experiment, built and ready to measure, 11.3 Measured: instruction fetch confirmed (2026-08-04, Plus 2 W), 11.4 Tier 2 measured: 53.2 ms, and every moved function shows (Plus 2 W), 11.5 Tier 3 measured, and the pattern is now the finding (Plus 2 W), 11.6 Tier 4, and where the tiering stops (Plus 2 W), 11. M5 — re-profile before choosing a lever (2026-08-02) (+25 more)

### Community 85 - "lexer.c"
Cohesion: 0.16
Nodes (31): Lexer, Token, TokenType, is_delimiter(), is_digit(), is_number_char(), is_space(), is_valid_number() (+23 more)

### Community 86 - "Turtle Trails (design)"
Cohesion: 0.06
Nodes (32): 10. Main loop and state order, 11.1 The procedure table, 11. Memory and performance budget, 12. Design boundaries, 13. Tests, 14. Implementation milestones, 15. As built: divergences from this design, 1. Theme (+24 more)

### Community 87 - "P5 — Multi-sprite turtles and the display pipeline (implemented)"
Cohesion: 0.06
Nodes (31): 10. Budgets, 11. Phasing, 12. Risks and open questions, 13. Rejected alternatives (summary), 1. Where the time goes today, 2.1 Tile-based dirty tracking, 2.2 DMA blit with a pipelined palette-expansion line buffer, 2.3 Refresh policy: automatic by default, manual on request (+23 more)

### Community 88 - "procedures.c"
Cohesion: 0.11
Nodes (27): atom_memo_bind_index(), atom_memo_bind_kind(), atom_memo_set_binding(), Token, resolve_word(), UserProcedure, find_procedure_index(), find_procedure_index_n() (+19 more)

### Community 89 - "lfs_storage.c"
Cohesion: 0.10
Nodes (19): LogoDirCallback, LogoStream, lfs_storage_fs_image_backup(), lfs_storage_fs_image_restore(), lfs_storage_list_directory(), lfs_storage_open(), lfs_stream_can_read(), lfs_stream_close() (+11 more)

### Community 90 - "keyboard.c"
Cohesion: 0.09
Nodes (22): repeating_timer_t, keyboard_get_key(), keyboard_init(), keyboard_key_available(), keyboard_peek_key(), keyboard_poll(), keyboard_set_background_poll(), keyboard_set_idle_callback() (+14 more)

### Community 91 - "mock_sdcard.c"
Cohesion: 0.12
Nodes (19): clear_root_cluster(), compute_fat_size(), fat32_image_format_mbr(), fat32_image_format_superfloppy(), write_boot_sector(), write_fsinfo(), write_initial_fat(), sd_error_t (+11 more)

### Community 92 - "test_galaxian.c"
Cohesion: 0.14
Nodes (28): assert_num(), assert_true(), load_file(), num(), run_frames(), run_scoring_frames(), seed_convoy(), start_level() (+20 more)

### Community 93 - "picocalc_storage.c"
Cohesion: 0.14
Nodes (27): fat32_get_cluster_size(), fat32_get_generation(), fat32_seek(), fat32_size(), LogoStream, file_context_stale(), logo_picocalc_dir_create(), logo_picocalc_dir_exists() (+19 more)

### Community 94 - "P8 — Sound: a stereo PSG synthesizer (design)"
Cohesion: 0.07
Nodes (29): 10. Rejected alternatives, 11. Resolved questions (user, 2026-07-10), 12.1 DMA read ring-wrap (engine, 2026-07-18), 12.2 LCD driver no longer masks interrupts (2026-07-19), 12.3 Audio IRQ priority above default (2026-07-19), 12. Hardware bring-up findings (2026-07-18/19), 1. What limits sound today, 2. The output hardware (+21 more)

### Community 95 - "main"
Cohesion: 0.08
Nodes (21): Lexer, eval_init(), proc_reset_execution_state(), lfs_t, LogoStorage, logo_lfs_storage_init(), main(), psram_verify() (+13 more)

### Community 96 - "mem_atom_cstr"
Cohesion: 0.18
Nodes (27): mem_atom_cstr(), Lexer, parse_list_body(), Evaluator, Result, Value, hostname_is_valid(), prim_tls_supported() (+19 more)

### Community 97 - "test_storage_router.c"
Cohesion: 0.07
Nodes (6): LogoEntryType, LogoStream, collect_cb(), make_stream(), setUp(), spy_reset()

### Community 98 - "primitives_sound.c"
Cohesion: 0.22
Nodes (26): Evaluator, LogoHardwareOps, LogoIO, Node, Result, SoundEvent, Value, is_noise_voice() (+18 more)

### Community 99 - "test_primitives_debug_control.c"
Cohesion: 0.07
Nodes (25): test_co_at_toplevel(), test_freeze_request_break_stops_execution(), test_freeze_request_waits_for_key(), test_go_label_not_found_in_procedure(), test_go_no_label(), test_go_with_label(), test_label_basic(), test_pause_at_toplevel_error() (+17 more)

### Community 100 - "host_storage.c"
Cohesion: 0.12
Nodes (17): LogoDirCallback, LogoStream, host_file_can_read(), host_file_close(), host_file_flush(), host_file_get_length(), host_file_get_read_pos(), host_file_get_write_pos() (+9 more)

### Community 101 - "test_costumes.c"
Cohesion: 0.19
Nodes (21): costume_delete(), costume_get(), costume_pool_free(), costume_put(), costumes_clear(), pool_release(), turtle_put_shape_data(), turtles_init() (+13 more)

### Community 102 - "properties.c"
Cohesion: 0.20
Nodes (23): Evaluator, Result, Value, prim_erprops(), prim_gprop(), prim_plist(), prim_pprop(), prim_pps() (+15 more)

### Community 103 - "test_primitives_debug.c"
Cohesion: 0.11
Nodes (23): proc_is_stepped(), proc_is_traced(), setUp(), tearDown(), test_step_and_trace_independent(), test_step_multiline_procedure(), test_step_nonexistent_gives_error(), test_step_pauses_execution() (+15 more)

### Community 104 - "test_lfs_storage.c"
Cohesion: 0.12
Nodes (18): Listing, bd_erase(), bd_prog(), bd_read(), lfs_block_t, lfs_off_t, lfs_size_t, LogoEntryType (+10 more)

### Community 105 - "host_hardware.c"
Cohesion: 0.09
Nodes (5): LogoHardware, host_network_tcp_connect(), init_winsock(), logo_host_hardware_create(), logo_host_hardware_destroy()

### Community 106 - "Code Review — 2026-07-02"
Cohesion: 0.08
Nodes (23): 1. Confirmed bug: `recycle` sweeps reachable data, 2.1 `primitive_find` is a linear `strcasecmp` scan (top optimization candidate), 2.2 `find_atom` is a linear scan of the whole atom table, 2.3 Smaller items, 2. Hot-path efficiency, 3. Robustness: `mem_cons` failures are silently ignored, 4.1 Minus sign after `)` — deliberate, documented, but a literal conflict, 4.2 Word equality case sensitivity — three-way inconsistency, needs a decision (+15 more)

### Community 107 - "Design: LittleFS internal filesystem + `/sd` FAT32 mount"
Cohesion: 0.08
Nodes (23): 10. Testing strategy, 11. Phased plan, 12. Decisions (resolved), 1. Goals, 2. Current architecture (baseline), 3. Flash layout — surviving flash-and-debug, 4. The PSRAM / QMI-safe flash-write path (do this FIRST), 5. LittleFS block device + configuration (+15 more)

### Community 108 - "Contributing"
Cohesion: 0.08
Nodes (23): About Logo, Additional Features for the PicoCalc, Advanced Logo, Beginning Logo, Building and Running, Contributing, Credits, Dependencies (+15 more)

### Community 109 - "primitives_http.c"
Cohesion: 0.25
Nodes (21): buf_appendf(), Evaluator, Result, Value, check_header_args(), ci_equal(), decode_chunked(), header_token_is_safe() (+13 more)

### Community 110 - "repl_evaluate_line"
Cohesion: 0.12
Nodes (22): Result, repl_count_bracket_balance(), repl_evaluate_line(), repl_find_end_token(), repl_next_bracket_depth(), repl_proc_def_append(), repl_restore_refresh(), repl_suggest_name() (+14 more)

### Community 111 - "The pick of five: plans"
Cohesion: 0.09
Nodes (23): Documentation, Done — `setpensize` / `pensize`, Implementation refinements (code-review leftovers), Language: big bets, Language: cheap wins (small primitives, high classroom value), Language: medium, P10 — Interpreter throughput, P1 — Host REPL stdin + CI (+15 more)

### Community 112 - "Arithmetic Operations"
Cohesion: 0.09
Nodes (23): abs, arctan, Arithmetic Operations, cos, difference, exp, form, int (+15 more)

### Community 113 - "Space Invaders in Pico Logo (design & implementation)"
Cohesion: 0.09
Nodes (22): 10. Why this is a good P5 acceptance test, 11. Deliverable, 1. The board, 2. Object representation — the central decision, 3. The alien formation on the canvas, 4. Collision routing — demons vs. the game loop, 5. Global events as demons, 6. Input (+14 more)

### Community 114 - "package.json"
Cohesion: 0.09
Nodes (21): categories, contributes, grammars, languages, description, devDependencies, @vscode/vsce, displayName (+13 more)

### Community 115 - "roadmap.md"
Cohesion: 0.21
Nodes (7): Build and test, Code structure, Constraints, Graphify, Project, Unit testing, Working guidelines

### Community 116 - "primitives.c"
Cohesion: 0.16
Nodes (18): primitive_by_index(), primitive_find(), primitive_find_n(), primitive_get_by_index(), primitive_get_count(), primitive_index_of(), primitive_is_output(), primitive_name_compare() (+10 more)

### Community 117 - "sdcard.c"
Cohesion: 0.27
Nodes (19): fat32_init(), sd_error_t, sd_card_init(), sd_cs_deselect(), sd_cs_select(), sd_error_string(), sd_init(), sd_read_block() (+11 more)

### Community 118 - "Managing your Workspace"
Cohesion: 0.10
Nodes (21): bury, buryall, buryname, erall, erase (er), ern, erns, erps (+13 more)

### Community 119 - "test_invaders.c"
Cohesion: 0.20
Nodes (19): assert_num(), assert_true(), load_file(), num(), run_frames(), run_scoring_frames(), start_level(), tearDown() (+11 more)

### Community 120 - "test_scaffold.c"
Cohesion: 0.12
Nodes (8): LogoStream, mock_stream_can_read(), mock_stream_close(), mock_stream_flush(), mock_stream_read_char(), mock_stream_read_chars(), mock_stream_read_line(), mock_stream_write()

### Community 121 - "eval.c"
Cohesion: 0.28
Nodes (19): EvalOpKind, Evaluator, FrameStack, Node, Result, Value, eval_get_frames(), eval_in_procedure() (+11 more)

### Community 122 - "test_help.c"
Cohesion: 0.12
Nodes (12): help_check_sorted(), help_contains_nocase(), help_lookup(), test_help_contains_nocase(), test_help_lookup_is_case_insensitive(), test_help_lookup_returns_null_for_unknown(), test_help_lookup_returns_text_for_known_primitive(), test_help_table_is_sorted() (+4 more)

### Community 123 - "test_tls_heap.c"
Cohesion: 0.19
Nodes (15): picocalc_tls_heap_setup(), tls_heap_calloc(), tls_heap_free(), tls_heap_init(), tls_heap_malloc(), setUp(), test_calloc_overflow_returns_null(), test_calloc_zeroes() (+7 more)

### Community 124 - "storage_router.c"
Cohesion: 0.18
Nodes (19): LogoDirCallback, LogoStream, cross_fs_move(), is_root(), router_dir_create(), router_dir_delete(), router_dir_exists(), router_file_delete() (+11 more)

### Community 125 - "read_map"
Cohesion: 0.17
Nodes (20): numf(), read_map(), read_slots(), step_tile(), test_calm_tiles_do_not_strand_bugs(), test_decoded_counts_match_the_encoded_words(), test_direction_deltas_and_opposites(), test_level_profiles_escalate() (+12 more)

### Community 126 - "File Management"
Cohesion: 0.11
Nodes (19): backup, cat, catalog, copyfile, createdir, dir? (dirp), directories, editfile (+11 more)

### Community 127 - "The Outside World"
Cohesion: 0.11
Nodes (19): env, key? (keyp), play, playing? (playingp), print (pr), readchar (rc), readchars (rcs), readlist (rl) (+11 more)

### Community 128 - "picocalc_flash.c"
Cohesion: 0.18
Nodes (15): m1_capture(), m1_equal(), picocalc_flash_erase(), picocalc_flash_program(), picocalc_flash_read(), picocalc_flash_selftest(), writable_m1(), bd_erase() (+7 more)

### Community 129 - "sound.c"
Cohesion: 0.18
Nodes (13): SoundEvent, SoundStatus, is_noise_voice(), __not_in_flash_func(), queue_empty(), queue_free(), sound_gate(), sound_init() (+5 more)

### Community 130 - "Galaxian in Pico Logo (design)"
Cohesion: 0.11
Nodes (18): 10. Main loop, 11. Risks / tuning expectations, 1. What Galaxian is, mechanically, 2. The board, 3. Object representation, 4. The convoy, 5. Divers — the new mechanic, 6. Shot vs. convoy: `colourunder`, not `over?` (+10 more)

### Community 131 - "test_bench_throughput.c"
Cohesion: 0.30
Nodes (16): bench_line(), calibrate_ns(), define_workspace(), frame_storage_cells(), load_game(), now_ms(), test_bench_expr_shapes(), test_bench_galaxian_play_frame() (+8 more)

### Community 132 - "primitives_control_flow.c"
Cohesion: 0.38
Nodes (16): Evaluator, Result, Value, eval_to_number(), prim_do_until(), prim_do_while(), prim_forever(), prim_ignore() (+8 more)

### Community 133 - "primitives_outside_world.c"
Cohesion: 0.35
Nodes (16): Evaluator, Result, Value, flush_writer(), prim_keyp(), prim_print(), prim_readchar(), prim_readchars() (+8 more)

### Community 134 - "frame_get_test"
Cohesion: 0.18
Nodes (16): frame_get_test(), frame_set_test(), var_get_test(), var_reset_test_state(), var_set_test(), var_test_is_valid(), test_reuse_clears_test_state(), test_set_test_false() (+8 more)

### Community 135 - "frame_stack_depth"
Cohesion: 0.17
Nodes (16): frame_stack_available_bytes(), frame_stack_depth(), frame_stack_is_empty(), Value, LOGO_HOT(), var_get_local_by_index(), var_local_count(), test_pop_all_frames() (+8 more)

### Community 136 - "clib.c"
Cohesion: 0.22
Nodes (14): logo_host_rename(), fat32_error_t, _close(), fat32_error_to_errno(), _fstat(), init(), _lseek(), _open() (+6 more)

### Community 137 - "Atom Garbage Collection: Implementation Plan"
Cohesion: 0.12
Nodes (15): Alternatives not selected, Atom allocator and collector, Atom Garbage Collection: Implementation Plan, Background: the "atoms are never freed" simplification, Collection behaviour, Documentation updates during implementation, Existing groundwork and prerequisite, Implementation (+7 more)

### Community 138 - "Input and Output to Files, Network Connections and Devices"
Cohesion: 0.12
Nodes (16): allopen, close, closeall, filelen, Input and Output to Files, Network Connections and Devices, open, reader, readpos (+8 more)

### Community 139 - "record_command"
Cohesion: 0.12
Nodes (16): mock_screen_fullscreen(), mock_screen_refresh_now(), mock_screen_set_refresh_auto(), mock_screen_splitscreen(), mock_screen_textscreen(), mock_text_clear(), mock_turtle_clear(), mock_turtle_draw() (+8 more)

### Community 140 - "LOGO_HOT"
Cohesion: 0.22
Nodes (13): apply_binary_op(), Evaluator, Node, Result, TokenType, Value, eval_expression(), is_number_string() (+5 more)

### Community 141 - "mem_set_cdr"
Cohesion: 0.20
Nodes (15): mem_set_cdr(), node_to_index(), append_to_list(), Lexer, Node, Token, parse_bracket_contents(), token_to_atom() (+7 more)

### Community 142 - "primitives_debug_control.c"
Cohesion: 0.28
Nodes (13): Evaluator, Result, Value, pause_check_continue(), pause_request_continue(), pause_reset_state(), prim_co(), prim_go() (+5 more)

### Community 143 - "ensure_wifi_initialized"
Cohesion: 0.16
Nodes (15): WifiState, ensure_wifi_initialized(), mdns_start(), picocalc_network_ping(), picocalc_network_resolve(), picocalc_network_set_hostname(), picocalc_wifi_connect(), picocalc_wifi_get_ip() (+7 more)

### Community 144 - "picocalc_network_tls_connect"
Cohesion: 0.18
Nodes (15): ntp_recv_callback(), picocalc_dns_callback(), picocalc_network_tcp_connect(), picocalc_network_tcp_listen(), picocalc_network_tls_connect(), ping_recv_callback(), poll_lwip_with_timeout(), tcp_connect_and_wait() (+7 more)

### Community 145 - "southbridge.c"
Cohesion: 0.32
Nodes (14): picocalc_get_battery_level(), picocalc_power_off(), sb_is_power_off_supported(), sb_read(), sb_read_battery(), sb_read_keyboard(), sb_read_keyboard_backlight(), sb_read_keyboard_state() (+6 more)

### Community 146 - "Design: `launch` background processes (P6)"
Cohesion: 0.13
Nodes (15): 10. Milestones, 11. Risks, 12. Decisions (gate closed 2026-07-12), 13. Alternatives rejected, 1. Goals, 2. Prior art (survey in multi-sprite-design.md §3/§8), 3. The model, 4. Feasibility: what the evaluator already gives us, and the one gap (+7 more)

### Community 147 - "host_console.c"
Cohesion: 0.26
Nodes (13): LogoConsole, LogoStream, host_input_can_read(), host_input_read_char(), host_input_read_chars(), host_input_read_line(), host_output_flush(), host_output_write() (+5 more)

### Community 148 - "logo_io_start_dribble"
Cohesion: 0.22
Nodes (14): logo_io_cleanup(), logo_io_is_dribbling(), logo_io_start_dribble(), logo_io_stop_dribble(), tearDown(), test_check_write_error_with_dribble(), test_dribble(), test_dribble_input() (+6 more)

### Community 149 - "Using the Logo Editor"
Cohesion: 0.14
Nodes (14): Block editing, Cursor motion, edall, edit (ed), Editing actions, edn, edns, end (+6 more)

### Community 150 - "WiFi Management"
Cohesion: 0.14
Nodes (14): Example, tls? (tlsp), wifi.connect, wifi.disconnect, wifi.hostname, wifi.ip, wifi.mac, WiFi Management (+6 more)

### Community 151 - "HTTP Server"
Cohesion: 0.14
Nodes (14): http.body, http.element, http.listen, http.method, http.path, http.query, http.remote, http.reqheader (+6 more)

### Community 152 - "primitives_conditionals.c"
Cohesion: 0.48
Nodes (11): eval_push_if(), Evaluator, Result, Value, prim_false(), prim_if(), prim_ifelse(), prim_iffalse() (+3 more)

### Community 153 - "Concurrent present on core 1 (design)"
Cohesion: 0.17
Nodes (12): 10. Tests, 1. Goal, 2. The evidence: core 0 sleeps through the present, 3. What it buys — and what it does not, 4. Why the usual memory objection does not apply, 5. Ownership model (sketch, deliberately unsettled), 6. M0 — the contention probe, and the gate, 7. Risks beyond the gate (+4 more)

### Community 154 - "HTTP server (design)"
Cohesion: 0.17
Nodes (11): 10. Decisions (resolved with the user), 1. Goal, 2. What already exists, 3. Primitive surface, 4. Execution model: a poll-driven pump, 5. Device interface changes (`devices/hardware.h`), 6. Core structure, 7. mDNS naming (added 2026-07-12) (+3 more)

### Community 155 - "MockCommandType"
Cohesion: 0.17
Nodes (12): MockCommandType, LogoPen, mock_turtle_dot(), mock_turtle_get_pen_state(), mock_turtle_set_bg_colour(), mock_turtle_set_pen_colour(), mock_turtle_set_pen_state(), mock_turtle_set_position() (+4 more)

### Community 156 - "Introduction"
Cohesion: 0.17
Nodes (12): A Further Note on Operations, Another Way to Talk about Procedures, Formal Logo, How to Think about Procedures You Define and their Inputs, How You Might Think about MAKE, How You Might Think about Quotes, Introduction, Logo Objects (+4 more)

### Community 157 - "record_command_float"
Cohesion: 0.18
Nodes (12): LogoRotationStyle, heading_to_radians(), mock_turtle_move(), mock_turtle_select(), mock_turtle_set_heading(), mock_turtle_set_rotation_style(), mock_turtle_set_scale(), mock_turtle_set_shape() (+4 more)

### Community 158 - "repl_line_starts_with_to"
Cohesion: 0.18
Nodes (11): repl_line_is_end(), repl_line_starts_with_to(), load_graphics_demo(), setUp(), test_repl_line_is_end_basic(), test_repl_line_is_end_false_cases(), test_repl_line_is_end_with_whitespace(), test_repl_line_starts_with_to_basic() (+3 more)

### Community 159 - "ms_to_datetime"
Cohesion: 0.36
Nodes (11): datetime_to_ms(), days_in_month_of_year(), ensure_software_clock_initialized(), get_current_epoch_ms(), is_leap_year(), ms_to_datetime(), picocalc_get_date(), picocalc_get_time() (+3 more)

### Community 160 - "Text and Screen Commands"
Cohesion: 0.18
Nodes (11): cleartext (ct), cursor, fullscreen (fs), refresh, refreshmode, setcursor, setrefresh, splitscreen (ss) (+3 more)

### Community 161 - "primitives_bitwise.c"
Cohesion: 0.53
Nodes (9): Evaluator, Result, Value, prim_ashift(), prim_bitand(), prim_bitnot(), prim_bitor(), prim_bitxor() (+1 more)

### Community 162 - "LogoStream"
Cohesion: 0.20
Nodes (10): LogoStream, mock_stream_can_read(), mock_stream_close(), mock_stream_flush(), mock_stream_read_char(), mock_stream_read_chars(), mock_stream_read_line(), mock_stream_write() (+2 more)

### Community 163 - "logo_hardware_init"
Cohesion: 0.22
Nodes (7): LogoHardware, LogoHardwareOps, logo_hardware_init(), LogoHardware, logo_picocalc_hardware_create(), logo_picocalc_hardware_destroy(), test_play_no_sound_engine_is_noop()

### Community 164 - "PR Review Checklist (CRITICAL)"
Cohesion: 0.22
Nodes (8): 1. Floating point — single precision only, 2. Static memory footprint, 3. Error handling conventions, 4. Logo semantics, 5. Project conventions, GitHub Copilot Instructions, PR Review Checklist (CRITICAL), What NOT to comment on

### Community 165 - "List Processing"
Cohesion: 0.22
Nodes (9): apply, crossmap, filter, find, foreach, List Processing, map, map.se (+1 more)

### Community 166 - "CLAUDE.md"
Cohesion: 0.25
Nodes (7): Build & Test, Code Structure, Constraints, graphify, How to work, Project, Unit Testing

### Community 167 - "as_httpd_conn"
Cohesion: 0.32
Nodes (8): MockHttpdConn, as_httpd_conn(), httpd_conn_read(), httpd_conn_write(), mock_network_tcp_can_read(), mock_network_tcp_close(), mock_network_tcp_read(), mock_network_tcp_write()

### Community 168 - "Appendix A: Useful Tools"
Cohesion: 0.25
Nodes (8): Appendix A: Useful Tools, arcr and arcl, circler and circlel, divisor?, Graphics Tools, Math Tools, Program Logic or Debugging Tools, sort

### Community 169 - "Appendix B: Parsing"
Cohesion: 0.25
Nodes (8): Appendix B: Parsing, Brackets and Parentheses, Delimiters and Spacing, Infix Procedures, Quotation Marks and Delimiters, The Minus Sign, Vertical Bars, Words

### Community 170 - "Modifying Procedures Under Program Control"
Cohesion: 0.25
Nodes (8): copydef, define, defined? (definedp), help, Modifying Procedures Under Program Control, primitive? (primitivep), primitives, text

### Community 171 - "Managing Various Files"
Cohesion: 0.25
Nodes (8): dribble, load, loadpic, Managing Various Files, nodribble, save, savel, savepic

### Community 172 - "HTTP Operations"
Cohesion: 0.25
Nodes (8): http.delete, http.get, http.header, HTTP Operations, http.patch, http.post, http.put, http.status

### Community 173 - "Tile Maps"
Cohesion: 0.25
Nodes (8): newmap, newtiles, settile, snaptile, stampmap, stamptile, tile, Tile Maps

### Community 174 - "Bitwise Operations"
Cohesion: 0.29
Nodes (7): ashift, bitand, bitnot, bitor, Bitwise Operations, bitxor, lshift

### Community 175 - "Variables"
Cohesion: 0.29
Nodes (7): local, localmake, make, name, name? (namep), thing, Variables

### Community 177 - "Pico Logo"
Cohesion: 0.33
Nodes (5): Building, Features, File Extensions, Installation, Pico Logo

### Community 178 - "Time Management"
Cohesion: 0.33
Nodes (6): date, setdate, settime, ticks, time, Time Management

### Community 179 - "JSON"
Cohesion: 0.33
Nodes (6): JSON, json.array, json.count, json.get, json.make, json.object

### Community 180 - "drain_tokens"
Cohesion: 0.33
Nodes (6): Lexer, drain_tokens(), test_fuzz_deeply_nested_brackets(), test_fuzz_many_consecutive_minus(), test_fuzz_many_quoted_words(), test_fuzz_many_small_tokens()

### Community 181 - "Bugs"
Cohesion: 0.40
Nodes (5): B6 — `penreverse` ignores pen size (always 1 px), Bugs, Fixed, Open, Reporting a bug

### Community 183 - "gen_ca_certs.py"
Cohesion: 0.83
Nodes (3): main(), split_pem_blocks(), subject_cn()

### Community 184 - "pandoc_slug"
Cohesion: 0.67
Nodes (3): main(), pandoc_slug(), Compute pandoc's auto_identifiers slug for a heading.

### Community 186 - "iteration_callback"
Cohesion: 0.67
Nodes (4): FrameHeader, FrameStack, iteration_callback(), stop_at_two()

### Community 187 - "mock_device_set_raster"
Cohesion: 0.67
Nodes (3): LogoTurtleRaster, mock_device_set_raster(), mock_turtle_get_raster()

## Knowledge Gaps
- **807 isolated node(s):** `dist.sh script`, `flash.sh script`, `name`, `displayName`, `description` (+802 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **10 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `run_string()` connect `run_string` to `eval_string`, `test_bench_throughput.c`, `reset_output`, `mem_word_ptr`, `define_proc`, `mem_is_nil`, `mem_atom`, `value_to_string`, `lexer_init`, `format_buffer_init`, `test_mock_device.c`, `mem_set_cdr`, `test_primitives_editor.c`, `proc_define_from_text`, `repl_line_starts_with_to`, `test_scaffold.h`, `test_primitives_json.c`, `logo_hardware_init`, `result_none`, `test_scaffold_setUp`, `test_checkrun.c`, `test_variables.c`, `test_primitives_files_load_save.c`, `error_format`, `test_primitives_conditionals.c`, `test_trails.c`, `test_primitives_files.c`, `test_primitives_hardware.c`, `set_mock_input`, `test_time.c`, `test_primitives_files_directory.c`, `test_primitives_outside_world.c`, `test_primitives_tilemap.c`, `demons_poll`, `test_sound.c`, `LOGO_HOT`, `test_galaxian.c`, `main`, `test_primitives_debug_control.c`, `test_primitives_debug.c`, `test_invaders.c`, `test_scaffold.c`, `eval.c`?**
  _High betweenness centrality (0.136) - this node is a cross-community bridge._
- **Why does `eval_string()` connect `eval_string` to `run_string`, `test_bench_throughput.c`, `reset_output`, `test_value.c`, `define_proc`, `mem_is_nil`, `mem_atom`, `value_to_string`, `mem_word_ptr`, `format_buffer_init`, `LOGO_HOT`, `mem_set_cdr`, `lexer_init`, `test_httpd.c`, `proc_define_from_text`, `test_primitives_http.c`, `test_scaffold.h`, `test_primitives_json.c`, `test_primitives_wifi.c`, `test_mock_fs.h`, `test_checkrun.c`, `test_variables.c`, `error_format`, `test_primitives_conditionals.c`, `test_trails.c`, `test_primitives_files.c`, `test_primitives_hardware.c`, `test_time.c`, `test_primitives_files_directory.c`, `test_primitives_outside_world.c`, `test_primitives_tilemap.c`, `demons_poll`, `test_sound.c`, `test_primitives_network.c`, `test_galaxian.c`, `main`, `mem_atom_cstr`, `test_invaders.c`, `test_scaffold.c`, `eval.c`, `read_map`?**
  _High betweenness centrality (0.117) - this node is a cross-community bridge._
- **Why does `mem_word_ptr()` connect `mem_word_ptr` to `run_string`, `eval_string`, `test_value.c`, `reset_output`, `primitives_outside_world.c`, `define_proc`, `mem_is_nil`, `mem_atom`, `value_to_string`, `format_buffer_init`, `LOGO_HOT`, `mem_set_cdr`, `primitives_debug_control.c`, `primitives.h`, `test_primitives_editor.c`, `mem_cons`, `test_httpd.c`, `proc_define_from_text`, `primitives_turtle.c`, `result_error_arg`, `test_primitives_http.c`, `httpd.c`, `test_primitives_json.c`, `test_primitives_wifi.c`, `result_none`, `test_variables.c`, `memory.c`, `test_primitives_files.c`, `test_primitives_hardware.c`, `set_mock_input`, `test_time.c`, `test_primitives_files_directory.c`, `test_primitives_outside_world.c`, `test_sound.c`, `test_primitives_network.c`, `primitives_json.c`, `LOGO_HOT`, `procedures.c`, `mem_atom_cstr`, `primitives_sound.c`, `properties.c`, `test_primitives_debug.c`, `primitives_http.c`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Are the 992 inferred relationships involving `run_string()` (e.g. with `define_workspace()` and `frame_storage_cells()`) actually correct?**
  _`run_string()` has 992 INFERRED edges - model-reasoned connections that need verification._
- **Are the 926 inferred relationships involving `eval_string()` (e.g. with `frame_storage_cells()` and `num()`) actually correct?**
  _`eval_string()` has 926 INFERRED edges - model-reasoned connections that need verification._
- **Are the 440 inferred relationships involving `mem_word_ptr()` (e.g. with `value_is_true()` and `LOGO_HOT()`) actually correct?**
  _`mem_word_ptr()` has 440 INFERRED edges - model-reasoned connections that need verification._
- **Are the 232 inferred relationships involving `mem_atom()` (e.g. with `LOGO_HOT()` and `parse_list()`) actually correct?**
  _`mem_atom()` has 232 INFERRED edges - model-reasoned connections that need verification._