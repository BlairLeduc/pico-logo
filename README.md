...
| `battlezone` | The 1980 vector tank game, in 3D: a wireframe plain that wraps, cubes and pyramids projected from four ground columns each, a mountain range and a crescent moon at infinity, and two treads on four keys -- `1`/`Q` the left tread, `0`/`P` the right, one key per tread per direction the way the cabinet's two sticks worked. Four enemies come at you in turn -- a tank, a missile that kills by arriving, a supertank and a saucer worth 5,000 -- and being hit shatters the periscope until your next tank. Radar, an engine idle that revs with the treads, an alarm whose tempo closes as the enemy does, and a top-ten score table. Needs the fast clock: it asks the board for 300 MHz and says so rather than playing badly if the board will not take it. |
