...
Gives _shapenumber_ the picture described by _shapespec_ as its shape. _shapenumber_ is in the range of 1 to 25. Shape 0 cannot be changed.
...
Stands for `set sh`ape. Sets the shape of each turtle you are talking to. _shapenumber_ 0 is the line-drawn turtle; slots 1 to 25 hold shapes you define, written with [`putsh`](#putsh) or captured with [`snapsh`](#snapsh) - the same kind of shape either way. Shapes 1 through 25 are empty when Logo starts, and a turtle wearing an empty shape shows nothing.
...
The rotation style applies to the shapes in slots 1 to 25; shape 0, the line-drawn turtle, always rotates. Shapes larger than about 22 pixels may lose their corners at diagonal headings with `"full`, since the drawn turtle is limited to 32 by 32 pixels.
...
```
curl -T asteroids http://picologo.local/asteroids
