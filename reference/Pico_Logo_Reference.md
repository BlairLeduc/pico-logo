...
Demons armed with [`when`](#when) while a file loads do not run until the whole file — and its `startup`, if any — has run. A demon's action can therefore call a procedure the file defines further down, or `load` another file.
