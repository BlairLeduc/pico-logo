...
void test_subprocedure_sees_superprocedure_inputs(void)
{
    ...
    if (r.status == RESULT_ERROR) {
        ...
        snprintf(msg, sizeof(msg), "outer failed with error %d: %s %s",
                 result_get_error_code(r),
                 result_get_error_proc(r) ? result_get_error_proc(r) : "(nil)",
                 result_get_error_arg(r) ? result_get_error_arg(r) : "(nil)");
...
void test_copydef_error_dest_is_primitive(void)
{
    ...
    TEST_ASSERT_EQUAL(RESULT_ERROR, r.status);
    TEST_ASSERT_EQUAL(ERR_IS_PRIMITIVE, result_get_error_code(r));
}
