...
void test_user_interrupt_stops_evaluation(void)
{
    ...
    TEST_ASSERT_EQUAL(ERR_STOPPED, result_get_error_code(r));
...
void test_go_with_label(void)
{
    ...
    if (r.status == RESULT_ERROR) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Error %d: proc=%s arg=%s", result_get_error_code(r),
                 result_get_error_proc(r) ? result_get_error_proc(r) : "(null)",
                 result_get_error_arg(r) ? result_get_error_arg(r) : "(null)");
...
void test_go_label_not_found_in_procedure(void)
{
    ...
    TEST_ASSERT_EQUAL(RESULT_ERROR, r.status);
    TEST_ASSERT_EQUAL(ERR_CANT_FIND_LABEL, result_get_error_code(r));
    // Verify the error message includes the label name
    TEST_ASSERT_EQUAL_STRING("nothere", result_get_error_arg(r));
}
...
void test_pause_at_toplevel_error(void)
{
    ...
    TEST_ASSERT_EQUAL(RESULT_ERROR, r.status);
    TEST_ASSERT_EQUAL(ERR_AT_TOPLEVEL, result_get_error_code(r));
}
