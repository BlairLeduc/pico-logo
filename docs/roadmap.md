...
`draw.board` alone 20 ms — a **19.5×** cut, the same factor the host showed,
as a purely interpreter-bound build should give. The first run caught two
...
With both fixed, the frame is **73.6 ms against P10 M1's 73.4** — simulation
48.6, drawing 24.8, present 0.25. So **the C map is frame-neutral, and
design §3.4's and [P10 §7's](interpreter-throughput-design.md) expectation
that it is what closes Turtle Trails is disproved.** Both misread P9 M0: it
...
P10's declined M4 and its rejected bytecode body would attack. A 0.25 ms
present in a 73.6 ms frame is the same finding from the other side: the wire
this design was sized against is not in the picture until something
scrolls.
