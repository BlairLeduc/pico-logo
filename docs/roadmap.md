...
| Interpreter throughput (games hit their frame budgets) | todo | Opened 2026-08-01 by P9's failed M0 gate, design drafted ([`interpreter-throughput-design.md`](interpreter-throughput-design.md)): the display was never the bottleneck — both shipped games run at ~9 fps and ~4 fps against a designed 25, and ~48 % of interpreter runtime is spent re-deriving facts that cannot change (word class re-lexed every evaluation, names resolved by `strncasecmp` every call). Memoise them on the interned atom. Target: Turtle Trails' `play.frame` under 40 ms, from 87.3 ms. M0 (benchmark) and M1 (word class, **1.25×** on the profiled loop, game frames −11/−12 % on the host) done 2026-08-01; M1 also found the 34 % to be a board-only figure, so M2 needs a fresh profile to size it. See [P10](#p10--interpreter-throughput) |
...
Status: **design drafted 2026-08-01; M0 and M1 done 2026-08-01** —
...
Neither game is home: Trails needs another **1.83×** and Checkpoint Run
**5.82×**. M2 targets a share half M1's, part of which is variable lookup that
cannot be atom-cached, so it will not close either gap. What can, for Trails,
is P9's C map — `tile.at` inside `step.bugs` is 59 % of the frame, 43 ms of
the 73.2, and removing it lands the frame near 30 ms.
